namespace :form_answers do
  ...
  task fix_2022_counties: :environment do
    @map = {}

    CSV.read("lib/tasks/temprary/county-2022-2023-migration-map.csv", headers: true).each do |csv|
      @map[csv[0]] = csv[1]
    end

    nominations = FormAnswer.all
    ...
    def change_county(key, nomination)
      ...
      mapped = @map[old_county]

      return if !mapped || old_county == mapped
